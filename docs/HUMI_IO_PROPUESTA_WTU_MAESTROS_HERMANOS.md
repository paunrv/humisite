...
| R4 | §8, §16 Test 4 | ✅ **Aprobado por Product (D3)**: con email, la llave es el email; sin email, la dedupe cae a **teléfono** y pide confirmación en pantalla ("este tutor ya existe con Gael"). Nunca se fusiona en silencio. | — |
| R5 | §18 | ✅ **Aprobado por Product (D4)**: §18 se consolida en **1 ADR** (roles/autorización por rol + vista `instructor_roster` + atribución de asistencia; parte del candidato ADR-F de PR #316) y **1 PDR** (Staff & Family Access MVP: allowlist D1, dedupe D3, cambio de email explícito, digest por guardian). `student_access` N:M no necesita ADR propio. | — |
